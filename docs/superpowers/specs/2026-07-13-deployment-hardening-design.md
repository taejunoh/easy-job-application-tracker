...
4. **Permission cleanup is retried.** The one local `connection` record may contain `pendingCleanupOrigins: string[]` in addition to `{ serverUrl, accessToken?, invalidated }`. Before attempting removal, server changes, failed newly granted pairings, `401` invalidation, and Disconnect persist the origin in this list. A false or rejected removal leaves it pending and displays a warning. Startup retries every pending origin and removes successful entries from the record.
