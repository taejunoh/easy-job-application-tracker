...
The extension requires Chrome 140 or newer. The [Chrome Storage API reference](https://developer.chrome.com/docs/extensions/reference/api/storage) labels the generic `StorageArea.setAccessLevel()` surface as Chrome 102+, but Chromium commit [`a8f1f337c692360aaec9470a0a91f965011d37a3`](https://chromium.googlesource.com/chromium/src/+/a8f1f337c692360aaec9470a0a91f965011d37a3) enabled `setAccessLevel()` for the `local` and `sync` storage implementations during the M140 development cycle. Because this extension specifically depends on `chrome.storage.local.setAccessLevel()`, its manifest floor follows that implementation milestone rather than the broader API-reference label. Its service worker and popup both set `chrome.storage.local` to `TRUSTED_CONTEXTS` before credential access so content scripts cannot read the access token. The popup fails closed when that access level cannot be established, and the service worker treats a missing method, a synchronous throw, and an asynchronous rejection identically by attempting credential purge.
...
The verified token is bound to its stored server origin and a monotonically increasing in-memory connection generation. The extension enforces these lifecycle invariants:

1. **A newer generation always wins.** Startup captures its generation before `storage.local.get`. It rechecks that generation inside the credential queue before restoring a snapshot and after permission or network awaits. Every authenticated request captures an immutable origin, token, and generation. Delayed startup reads, verification responses, `401` responses, application saves, and keyword-analysis responses cannot mutate credential state, render stale analysis, or revive application-specific links after a newer connect, invalidation, or disconnect.
2. **Connect is one credential transaction.** The exact-origin permission request begins synchronously in the Connect click handler to retain Chrome's user gesture. Permission result handling, token verification, commit-time `permissions.contains`, record commit, old-origin cleanup, and the final status message execute as one serialized credential operation. The token is not stored when commit-time host access is absent; the popup asks the owner to click Connect again rather than attempting an asynchronous permission request without a user gesture. `Connected` is rendered only while the committed generation remains the active connection.
3. **Explicit disconnect is durable.** Before local persistence, Disconnect writes a secret-free `{ serverUrl, invalidated: true }` tombstone to `storage.session` and keeps the same tombstone in memory if session access fails. It then writes an invalidated local record and falls back to removing `connection`, `serverUrl`, and `accessToken` if that write fails. If both local operations fail, the session or in-memory tombstone prevents the stale token from becoming active; a later popup retries the purge without verifying the token. A successful new pair must clear an existing tombstone before storing its token.
4. **Permission cleanup is retried and idempotent.** The one local `connection` record may contain `pendingCleanupOrigins: string[]` in addition to `{ serverUrl, accessToken?, invalidated }`. Before attempting removal, server changes, failed newly granted pairings, `401` invalidation, and Disconnect persist the origin in this list. Cleanup checks `permissions.contains` first: a confirmed absent permission is already clean, while a rejected or synchronously throwing state check fails closed. If `permissions.remove` returns false, a second confirmed-absent check is also clean. This lets startup erase a marker after permission removal succeeded but marker persistence previously failed. A false or rejected cleanup that cannot confirm absence remains pending and displays a warning.

Connect always removes its new active origin from inherited cleanup markers before storing the verified pair, so reopening never removes permission required by that active connection. It carries every other pending origin forward and retries each independently; one failure does not discard another marker. If a session-only tombstone belongs to a different origin, Connect transfers that origin into the new record's pending cleanup list before clearing the tombstone. Startup tombstone handling uses the already-updated working record rather than the stale storage snapshot, so successful cleanup is not reinserted.

Startup does not treat a stored token as active until both `permissions.contains` and `/api/auth/verify` succeed. Revoked host permission invalidates or purges the stored credential without a network request. A network failure or `403` keeps the credential only in an inactive, purgeable pending state: authenticated product APIs cannot use it, while Disconnect remains available to remove it. Legacy top-level credential cleanup return values are checked and retried during both startup migration and invalidation.

Disconnect clears the token and any application-specific tracker link, retains the server URL as a draft, persists the invalidated record, and attempts to remove its host permission. Storage, session-tombstone, or permission cleanup failures leave the current popup disconnected and are shown explicitly; they never restore a credential. If setting `TRUSTED_CONTEXTS` fails, both popup and background attempt to purge all known credential keys. The popup never reads or uses the secret in that state and explicitly warns if the purge also fails.
